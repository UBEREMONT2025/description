...
**Решения:**
- Способ входа: телефон + СМС.
- В МВП требуется КЭП/УЭП для актов и договоров.
...
**Справка:** целевой уровень доступности — это процент времени, когда
система работает без простоев (например, 95% или 99%).

**Решения:**
- Пиковая нагрузка: до 50 заявок в день.
- Целевой уровень доступности (SLA): 95% (до 36,5 часов простоя в год).
...
**Решения:**
- WEB размещаем в облаке.
- Bitrix24 (коробка) и 1С — on‑prem.
- Цикл релизов: раз в 2 недели.
...
**Решения:**
- Первая итерация МВП (тендеры: дизайнеры и управляющие) — до 15.02.
- Критичный риск: сроки. Причины — объем разработки в Битрикс и риски
  нестабильности после обновлений Битрикс.
...
**Решения:**
- Приоритет интеграций: сначала WEB ↔ Битрикс24 ↔ 1С, затем SMS, ЭП, банк,
  Wazzup, телефония.
- Архитектуру финально утверждает руководитель разработки.
...
## Сводка решений (одна страница)
**МВП и процессы**
- МВП включает полный дизайн‑проект: тендер -> договор -> этапы -> акты.
- Тендеры: дизайнеры и управляющие включены в МВП.
- Закупки/логистика вынесены на следующий этап.
- Этапы подтверждает ПМ; замечания — отдельный процесс.

**Данные и доменная модель**
- Источник правды по проекту и этапам: Битрикс24.
- Сущности МВП: Лид/Сделка, Клиент, Проект, Участник проекта, Тендер, Отклик,
  Дизайн‑этап, Договор, Акт, Платёж.
- Платежи по акту: частичные (50% предоплата + 50% постоплата).

**Интеграции и синхронизация**
- Режим синхронизации: WEB ↔ Битрикс24 до 10–30 сек;
  Битрикс24 ↔ 1С до 1–5 мин.
- Строго синхронные операции: оплата, подписание актов, подписание договоров.
- Интеграции и очереди реализуем внутри БФФ (Laravel/CURSOR).
- Внешние системы: ЭП, Wazzup, банк, email, SMS, телефония.

**Файлы и хранение**
- МВП: файлы в Битрикс.Диск, доступ по прямым ссылкам.
- Хранение: 3 года; объем на объект до 1,2 ГБ.
- Форматы: PDF, DWG, изображения, DOCX, XLSX.
- Лимит размера файла: до 100 МБ.
- Офлайн‑доступ: предзагрузка по запросу пользователя (для объектов
  с подписанными договорами).

**Безопасность и доступ**
- Вход: телефон + СМС.
- КЭП/УЭП обязательна для актов и договоров в МВП.

**НФТ**
- SLA: 95% (до 36,5 часов простоя в год).
- Пиковая нагрузка: до 50 заявок в день.

**Развертывание и релизы**
- WEB — облако; Bitrix24 и 1С — on‑prem (коробки).
- Релизы: раз в 2 недели.

**Дорожная карта и риски**
- Первая итерация МВП (тендеры дизайнеры/управляющие): до 15.02.
- Критичный риск: сроки (объем разработки в Битрикс и риски обновлений).

**Валидация**
- Приоритет интеграций: WEB ↔ Битрикс24 ↔ 1С, затем SMS, ЭП, банк, Wazzup,
  телефония.
- Финальное утверждение архитектуры: руководитель разработки.

**Открытые вопросы**
- Провайдеры для ЭП, банка, email, SMS и телефонии.
- Техническая схема интеграции Wazzup (API/вебхуки) и каналы.
